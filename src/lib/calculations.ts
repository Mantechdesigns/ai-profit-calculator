...
// Revenue tier sanity check ranges
...
export function calculateAudit(
  ...
  // Adjust rounding so pillars sum exactly to totalLeak
  const pillarSum = pillars.reduce((s, p) => s + p.leakAmount, 0);
  if (pillarSum !== totalLeak && pillars.length > 0) {
    // Add/subtract the difference from the biggest pillar
