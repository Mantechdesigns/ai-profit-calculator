...
import { sendWebhookYT } from './lib/webhookYT';
import { trackPageView, trackQuizStart, trackLeadCapture } from './lib/metaPixel';
import WelcomeScreenYT from './components/WelcomeScreenYT';
import QuizSection from './components/QuizSection';
import RevenueInput from './components/RevenueInput';
import LoadingAnimation from './components/LoadingAnimation';
import ResultsBlurGateYT from './components/ResultsBlurGateYT';
import CalendarBookingPageYT from './components/CalendarBookingPageYT';

type AppStep = 'welcome' | 'quiz' | 'revenue' | 'loading' | 'blur-gate' | 'booking';

export default function AppYT() {
  ...
  const handleUnlock = (name: string, email: string) => {
    ...
    if (auditResult) {
      ...
      // Fire webhook (fire-and-forget) — tagged as YouTube source
      sendWebhookYT(name, email, auditResult).catch((err) =>
  ...
  return (
    <div className="min-h-screen bg-bg-primary">
      {step === 'welcome' && <WelcomeScreenYT onStart={handleStart} />}
